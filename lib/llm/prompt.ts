...
16. 【理解補助セクション — ニュースの背景 (backgroundContext)】この記事を理解するために最低限必要な制度・経緯を、日本の一般的なビジネス読者が理解できる日本語で200〜300字程度に整理してください。原文または参照資料で確認できる事実のみを使用し、評価・意見・推測・将来予測・「〜に違いない」「〜とみられる」等の推測表現・政府や企業の意図の推測・日本の制度との不正確な類推を含めないでください。「これまでどうだったか」「何が問題だったか」「なぜ今回の発表が行われたか」「今回何が変わったか」が分かるように書いてください。参照資料から経緯が確認できない場合は null を返してください。
17. 【理解補助セクション — 日本企業への影響 (japanBusinessImpact)】原文および参照資料に基づき、日本企業への直接的または間接的な影響を100〜180字程度で説明してください。どのような企業が対象となり得るか、インド進出済みの日系企業への影響が直接的か間接的か、企業が確認すべき事項があるかを整理してください。根拠がない場合は推測で補完せず、「現時点で公表されている情報からは、日本企業への直接的な影響は確認できません。」のように明記してください。法務・税務・投資・事業判断に関する断定的な助言(「必ず対応が必要」等)や根拠のない影響予測は行わないでください。毎回無理に具体的な影響を作らないでください。
...

{
  ...
  "backgroundContext": "ニュースの背景(200〜300字程度。参照資料で確認できる事実のみ。確認できなければnull)",
  "japanBusinessImpact": "日本企業への影響(100〜180字程度。根拠がなければ『直接的な影響は確認できない』旨を明記)",
  "keywords": [
