...
6. 「日本企業への示唆」セクションは、ソース記事に含まれない独自の分析・解釈のみで構成してください。最も重要な1つの視点に絞り、示唆は1件だけ提示してください。単なる行動指示ではなく、「記事中の事実 → 日本企業への作用経路 → 判断・行動」の因果を具体的に記述してください。
...
8. 【企業名・固有名詞は実名で明記 — 抽象化禁止】読者は日本企業の実務担当者で、「どの企業・誰・どの製品か」が最も価値ある情報です。ソースで特定できる企業名・人名・製品名・ブランド名は、タイトルだけでなく本文と「日本企業への示唆」にも実名のまま記載してください。「国内最大手」「大手IT企業」「ある自動車メーカー」のように一般名詞へぼかすことを禁止します(例:「国内最大手の自動車メーカー」ではなく「マルチ・スズキ」と明記)。特に日系資本が関わる企業(例:スズキ系のマルチ・スズキなど)は読者の関心が高いため必ず実名を出してください。ソースに明記がなく特定できない場合に限り一般的表現を許容します。著作権を理由に固有名詞を避けてはいけません(事実は保護対象外)。
...
10. 【本文と示唆の接続】「日本企業への示唆」で企業名・業界・数値を具体化する場合、その企業名・業界・数値を本文で先に説明してください。示唆で新しい事実や固有名詞を突然導入してはいけません。
...
- 【文体統一 — 厳守】summary(本文)・implications(示唆)の全文を通じて「だ・である調」に統一してください。「です・ます調」との混在は絶対に禁止です。文末は「〜だ」「〜である」「〜した」「〜している」「〜とみられる」「〜という」等にしてください。「〜です」「〜ます」「〜でしょう」「〜ください」で終わる文は使用しないでください。
...

{
  ...
  "implications": [
    "示唆(日本企業の動き方に関する独自分析を1件のみ、120〜220字。記事中の事実→作用経路→具体的判断・行動を含める)"
  ],
...
sourceUsage には、本文で使った資料番号と、その資料だけから採用した具体的事実を記載してください。factsUsed が空になる資料は sourceUsage と referenceUrls の両方から除外してください。
