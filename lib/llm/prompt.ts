...
13. 【本文文字数 — 厳守】summary は500字前後にしてください。許容範囲は480〜540字です。執筆後に必ず自分で文字数を確認し、480字未満なら具体的事実・背景を補い、540字超なら背景説明・一般論・重複表現を削ってください。
...
15. 【インド式数値単位の換算 — 厳守】lakh / crore は必ず正しく日本式へ換算してください: 1 lakh = 10万、1 crore = 1,000万 = 0.1億、1 lakh crore = 1兆。したがって「Rs 30,000 crore」は「3,000億ルピー」であり、「30,000億ルピー(=3兆)」と書くのは10倍の誤りで絶対禁止です。billion は10億($40 billion = 400億ドル。「40億ドル」は100倍の誤り)。インド式桁区切り(例: 3,26,41,704)はそのまま転記せず日本式(約3,264万)に読み替えてください。円換算を付す場合は概算レート(1ルピー≈1.7円、1ドル≈150〜165円)で桁が合うか必ず自己検算し、検算が合わない換算・レート不明の換算は書かないでください。
16. 【制作痕跡・自己言及の禁止】「読者想定：」「編集ノート：」「(表記訂正)」「（事実出典：[1]）」「（参考N）」「（40〜80字）」のような指示文・制作過程の断片を出力に残してはいけません。「編集部の現場取材では」「筆者は登記記録を確認した」など実際には行っていない取材・検証行為を主張する文、および当媒体自身のサービス宣伝文も禁止です。
...

{
  ...
  "summary": "日本語記事本文(500字前後。厳守範囲480〜540字。段落可。ソース名への言及を一切含めず、独立した記事として自然に読める文体で書く)",
  "implications": [
    "本記事のまとめ1(50字以内)",
    "本記事のまとめ2(50字以内)",
    "本記事のまとめ3(50字以内)"
