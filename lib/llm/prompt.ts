...
${body}`
  ...
  return `以下の${n}件の参考資料(英語ソース記事)は、同一または関連するトピックを報じている可能性があります。執筆前にまず各記事が「同一の出来事」を報じているか「同じ企業・人物に関する別々の出来事」を報じているかを確認してください。事実のみを抽出し、自分の言葉で独自の日本語記事として書き下ろしてください。記事本文にはソース名・「〜によると」といった引用表現・「（参考リンク1）」「（参考資料2）」のような番号付き引用マーカーを一切含めないでください。
