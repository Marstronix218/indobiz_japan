...
6. 「本記事のポイント」(implications) は、読者が最初に読む要点として3件ちょうど提示してください。各項目は40〜90字の具体的な一文にし、読者が「何が起きたか」を理解できるように書いてください。見出し語や固有名詞の羅列(悪い例:「EPFOが恩赦制度を導入」「企業向け制度」「6か月間実施」)は禁止です。「何がどう変わり、誰に関係するか」が読み取れる文にしてください(良い例:「過去の手続き不足がある企業でも、6か月以内に申請すれば是正できる」)。90字を超える長い分析文も書いてはいけません。
...
15. 【インド式数値単位の換算 — 厳守】lakh / crore は必ず正しく日本式へ換算してください: 1 lakh = 10万、1 crore = 1,000万 = 0.1億、1 lakh crore = 1兆。crore を億へ直すときは元の数値を10で割り、billion を億へ直すときは元の数値を10倍してください。したがって「Rs 30,000 crore」は「3,000億ルピー」であり、「30,000億ルピー」と書くのは10倍の誤りです。「$40 billion」は「400億ドル」であり、「40億ドル」と書くのは10分の1の誤りです。million は100万、trillion は1兆です。複合表記も桁ごとに検算してください(例: Rs 8,924 crore = 892億4,000万ルピー、Rs 38,682 crore = 3,868億2,000万ルピー、$21.3 billion = 213億ドル)。インド式桁区切り(例: 3,26,41,704)はそのまま転記せず日本式(約3,264万)に読み替えてください。別通貨への換算は、入力資料に換算額または換算レートが明記されている場合だけ記載してください。円換算を付す場合も入力資料のレートで桁が合うか自己検算し、検算が合わない換算・レート不明の換算は書かないでください。
15A. 【会計年度表記 — 厳守】FY26 / FY27 などの明示された年度番号を変えないでください。原文が current financial year / last financial year としか書いていない場合は、入力資料だけで対応年度を確定できない限り「今会計年度」「前会計年度」のまま書き、推測で「2026年度」などの番号を付けないでください。インドの「FY27」は2026年4月〜2027年3月を指すため、日本語では「2026/27年度(FY27)」と表記してください。
15B. 【制度・契約・施行状態の保持 — 最重要】ソースが draft / proposed / consultation / public comments / recommendation / guidance / 発効予定 / 施行予定 としているものを、確定済み規則・承認済み制度・現行の義務・発効済みとして書き換えてはいけません。草案中の shall / must は「草案が求めている内容」であり、現在の義務ではありません。タイトルと本文の両方に「草案」「案を公表」「意見募集」「○月○日発効予定」など基準日時点の状態を明記してください。発効済みと書けるのは、入力資料に発効を確認する記述がある場合だけです。
15C. 【決算指標の区別】reported / statutory（報告値）と adjusted / underlying（調整後値）、純利益と営業利益、前年同期比と前四半期比を混同しないでください。タイトルに丸めた増減率を書く場合も、本文では資料にある正確な率と対象指標を明示し、別指標の数値を同じ利益として接続しないでください。
16. 【理解補助セクション — ニュースの背景 (backgroundContext)】この記事を理解するために最低限必要な制度・経緯を、日本の一般的なビジネス読者が理解できる日本語で180〜220字程度に整理してください。原文または参照資料で確認できる事実のみを使用し、評価・意見・推測・将来予測・「〜に違いない」「〜とみられる」等の推測表現・政府や企業の意図の推測・日本の制度との不正確な類推を含めないでください。「これまでどうだったか」「何が問題だったか」「なぜ今回の発表が行われたか」「今回何が変わったか」が分かるように書いてください。参照資料から経緯が確認できない場合は null を返してください。
17. 【理解補助セクション — 日本企業への影響 (japanBusinessImpact)】原文および参照資料に基づき、日本企業への直接的または間接的な影響を180〜220字程度で説明してください。どのような企業が対象となり得るか、インド進出済みの日系企業への影響が直接的か間接的か、企業が確認すべき事項があるかを整理してください。backgroundContext と両方を返す場合は、2つの文字数差を40字以内にしてください。根拠がない場合は推測で補完せず、「現時点で公表されている情報からは、日本企業への直接的な影響は確認できません。」のように簡潔に明記してください。この根拠不足を示す定型文だけは文字数目安の例外です。法務・税務・投資・事業判断に関する断定的な助言(「必ず対応が必要」等)や根拠のない影響予測は行わないでください。毎回無理に具体的な影響を作らないでください。
18. 【理解補助セクション — キーワード解説 (keywords)】この記事を理解するために不可欠で、日本の一般的なビジネス読者には分かりにくい専門用語を最大4語(原則2〜4語)抽出してください。対象はインド固有の制度・法律・政府機関・規制当局・略称などに限り、一般的なビジネス用語は掲載しないでください。各用語は公的資料または参照資料中の記述で確認できる範囲に基づき50〜120字程度で定義し、独自の解釈・評価・予測・影響分析・法的効果の断定・日本の制度との類推(「日本の○○に相当する」は原則禁止)を行わないでください。十分な根拠がない用語は掲載せず、該当する用語がない場合は空配列 [] を返してください。
19. 【理解補助セクション — 画像キャプション (imageCaption)】メイン画像は imagePrompt から生成される匿名的なイメージ写真です。その写真に写る対象と記事との関係だけを、余裕を持って50〜80字程度の日本語1文で説明してください。写真から確認できない具体的事実(企業名・人名・日付・数値)や記事内容の解説を過剰に含めないでください。適切なキャプションが書けない場合は null を返してください。
19A. 原文が企業・項目を列挙していても、「2社」「3分野」のように原文に明記されていない件数を計算して追加しないでください。企業名・項目名をそのまま書いてください。
20. 【制作痕跡・自己言及の禁止】「読者想定：」「編集ノート：」「(表記訂正)」「（事実出典：[1]）」「（参考N）」「（40〜80字）」のような指示文・制作過程の断片を出力に残してはいけません。「編集部の現場取材では」「筆者は登記記録を確認した」など実際には行っていない取材・検証行為を主張する文、および当媒体自身のサービス宣伝文も禁止です。
...

{
  ...
  "implications": [
    "本記事のポイント1(40〜90字の具体的な一文)",
    "本記事のポイント2(40〜90字の具体的な一文)",
    "本記事のポイント3(40〜90字の具体的な一文)"
  ],
  "backgroundContext": "ニュースの背景(180〜220字程度。参照資料で確認できる事実のみ。確認できなければnull)",
  "japanBusinessImpact": "日本企業への影響(180〜220字程度。背景との文字数差は40字以内。根拠がなければ短く『直接的な影響は確認できない』旨を明記)",
  "keywords": [
    {"term": "用語(例: EPFO)", "fullName": "正式名称(なければnull)", "definition": "50〜120字程度の定義"}
  ],
  "imageCaption": "画像キャプション(50〜80字程度を狙った1文。書けなければnull)",
  "industryTags": ["automotive|semiconductor|machine_tools|food|chemicals|energy|logistics|agriculture|steel|education|entertainment|talentのうち該当するもの0〜3個。原油・石油・電力・天然ガスはchemicalsではなくenergy"],
