...
15A. 【制度・契約・施行状態の保持 — 最重要】ソースが draft / proposed / consultation / public comments / recommendation / guidance / 発効予定 / 施行予定 としているものを、確定済み規則・承認済み制度・現行の義務・発効済みとして書き換えてはいけません。草案中の shall / must は「草案が求めている内容」であり、現在の義務ではありません。タイトルと本文の両方に「草案」「案を公表」「意見募集」「○月○日発効予定」など基準日時点の状態を明記してください。発効済みと書けるのは、入力資料に発効を確認する記述がある場合だけです。
15B. 【決算指標の区別】reported / statutory（報告値）と adjusted / underlying（調整後値）、純利益と営業利益、前年同期比と前四半期比を混同しないでください。タイトルに丸めた増減率を書く場合も、本文では資料にある正確な率と対象指標を明示し、別指標の数値を同じ利益として接続しないでください。
