...
A. 知財・著作権リスク
  ...
  - 1本の参考記事が複数の話題・論説を含む場合、生成記事がその中の一貫したニュース価値のある一部に焦点を絞ることは許容する。参考記事内の全段落・全論点・全引用を網羅する必要はなく、採用部分が本文で実質的に使われていれば「未使用参考記事」と判定しない
  - referenceUrls に、同一記事のGoogle Newsリダイレクト、転載、同一タイトルが独立ソースとして重複掲載されていないか。なお【参考記事(入力資料)】同士の重複は生成側では修正できないため、referenceUrls 側で重複が排除されていれば問題として挙げず、REVISION理由にもしないこと
...
C. ニュース価値
  ...
  - 差別的・侮蔑的な表現や属性集団への一般化は、原文の主要人物による発言であっても再掲を要求しない。ニュースの理解に必要なら、中立的な要約で対立の事実を伝えることを優先する
...
D. 文章品質
  ...
  - logistics は物流事業だけでなく、高速鉄道・鉄道インフラ・貨物鉄道・港湾・輸送回廊などの交通インフラも含む。これらが記事の主要テーマなら logistics は整合しており、許可外の「交通」「インフラ」タグへの置換を要求しない
  ...
  - 理解補助セクション(ニュースの背景・日本企業への影響・キーワード解説)が出力されている場合、参考記事本文または公的に確認できる事実の範囲に収まっているか。編集部の意見・価値判断・将来予測・「〜に違いない」等の推測表現・法務税務投資の断定的助言・「日本の○○に相当する」等の安易な類推が含まれていればREVISION。フィールドがnull・空であること自体は問題にしない(無理に生成させない)
  - ニュースの背景と日本企業への影響が両方ある場合、原則として各180〜220字程度、文字数差40字以内を目安に情報量が釣り合っているか。ただし、日本企業への直接的な影響を確認できない旨だけを簡潔に示す場合は短くてもよい
E. 文脈の一貫性
...
- REVISION の判定理由と修正指示は、再生成で変えられる範囲(本文・ポイント・理解補助セクション・referenceUrls・sourceUsage・タグ)に限ること。入力資料そのものの欠陥(参考記事同士の重複・本文の不足・システム側のリスト整理)は再生成では直せないため、生成物側が適切に対処していれば判定理由にしない。「〜の余地がある」程度の改善提案しか残っていない場合はPASSとする
- 参考記事の一部に焦点を絞ったこと、侮蔑的な引用を省略したこと、許可された分類体系に専用タグがないことだけを理由にREVISIONへしない。タイトルと本文が焦点内で一貫し、採用した事実が資料で確認できればPASSとする
...
- backgroundContext・japanBusinessImpact・keywords も出力スキーマどおり再生成してください。参照資料で確認できる事実のみを使い、根拠が足りないフィールドは null(keywordsは空配列)にしてください。
- backgroundContext と japanBusinessImpact は各180〜220字程度とし、両方を返す場合の文字数差を40字以内にしてください。ただし、日本企業への直接的な影響を確認できない旨だけを簡潔に示す場合は例外です。
...
- 列挙された企業・項目から「2社」「3分野」などの件数を独自に計算せず、原文に件数が明記されていなければ固有名詞をそのまま書いてください。
