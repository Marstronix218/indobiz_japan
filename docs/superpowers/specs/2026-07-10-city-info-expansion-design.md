...
これらは Claude が `climate.json` の観測値を読んで判断し、`data.ts` に記述する。降水量の閾値から機械的に導出すると、年中穏やかで「避ける月」が存在しないベンガルールのような都市で不自然な結果になるため、スクリプトでは導出しない。`climate.json` は観測値のみを持ち、判断を含まない。
...
執筆と検証を同じ工程にすると、書いた内容を追認するだけになり検証が機能しない。そのため二段階に分ける。

**第1パス（執筆）** — 都市ごとに `WebSearch` で裏を取りながら `data.ts` を書く。

**第2パス（独立検証）** — 都市の `data.ts` が書き上がった後、**執筆時の検索結果を参照せずに**、記述された固有名詞・数値を1件ずつ独立に再検索して突き合わせる。対象は次の通り。

- 病院名 — 実在するか。その都市にあるか。
- 日本人学校・日本人会 — 実在するか。閉鎖されていないか。
- 家賃レンジ — 独立した情報源で桁が一致するか。
- 直行便の有無 — 現在も就航しているか。
- 駐在エリア名 — 実在する地名か。

判定は3値とする。**確認できた**ものは残す。**矛盾した**ものは修正するか削除する。**確認できなかった**ものは削除する（残さない）。

検証結果は `docs/city-fact-check-2026-07.md` に、項目・判定・出典URL の表として記録し、コミットする。後から「この家賃はどこから来たのか」を追跡できるようにするため。この監査ログの存在自体が、次に誰かがデータを更新するときの基準になる。

第2パスで削除が発生した結果、都市によって生活情報の密度に差が出る。密度を揃えるために推測を書くことはしない。

### 3. 気候データ（静的・実測由来）

`scripts/fetch-city-climate.mjs` は実装時に Claude が一度だけ実行し、出力をコミットする（CI やビルドからは呼ばれない）。Open-Meteo Archive API から各都市の 2015–2024 年の日次データ（最高気温・最低気温・降水量）を取得し、月別に集計して `lib/cities/climate.json` に書き出す。APIキー不要。緯度経度をカンマ区切りで渡せるため9都市を1リクエストで取得できる。
...
なお、テストが守れるのは**構造**であって**事実**ではない。「存在しない病院名が書かれている」はテストでは検出できない。事実の正しさを担保するのは §2 のダブルチェックと `docs/city-fact-check-2026-07.md` であり、この2つは別の役割を持つ。
...
- `docs/city-fact-check-2026-07.md` — 第2パスの検証ログ（項目・判定・出典URL）
...
最も重い作業はコードではなく、9都市分の生活情報を裏取りしながら執筆・検証する部分である。実装計画では都市ごとに独立したタスクとして分割し、各タスクは「第1パス（執筆）→ 第2パス（独立検証）→ 検証ログ追記」までを1単位とする。執筆だけ9都市まとめて済ませてから検証に入ると、検証が形骸化する。

人間が原稿を書く工程は存在しない。すべて Claude が執筆し、Claude が独立検証する。
